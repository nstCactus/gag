...
function at($msg = '')
{
    ...
    $atList[] = array('msg' => $msg, 'time' => $t);
    if (count($atList) > 1) {
        echo('AT: ' . $msg . ' / from last: ' . round(($t - @$atList[count($atList) - 2]['time']) * 1000) . ' / from start: ' . round(($t - @$atList[0]['time']) * 1000) . '<br>');
    }
}

at('start (' . __FILE__ . ' l:' . __LINE__ . ')');
 ...
 */
//@formatter:off
define('CONFIG_BASE_URL',   dirname(dirname($_SERVER['PHP_SELF'])));
define('CONFIG_WEBROOT',    realpath(dirname(dirname(__FILE__))) . '/');
