...
Control.DatePicker.prototype = {
	initialize: function(element, options) {
		...
		this.i18n = new Control.DatePicker.i18n(options && options.locale ? options.locale : 'fr_FR');
...
//		this.options.currentFormat = this.options.timePicker ? this.options.dateTimeFormat : this.options.dateFormat;
		...
		if(this.element.value == "")
			this.element.value = DateFormat.format(new Date(), this.options.currentFormat);
	...
	show: function () {
		if (!this.pickerActive) {
			...
			*/
			...
			var pickerTop = /MSIE/.test(navigator.userAgent) ? (pos[1]) + 'px' : (pos[1] - 1) + 'px';
			...
			this.datepicker.element.style.left = '0px';			
