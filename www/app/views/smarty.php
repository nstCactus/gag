...
class SmartyView extends View
{
	...
	function __construct (&$controller)
	{
		...
		$this->Smarty->plugins_dir[] = APP.'vendors'.DS.'smarty'.DS.'plugins'.DS;
		$this->Smarty->plugins_dir[] = VIEWS.'smarty_plugins'.DS;
		$this->Smarty->plugins_dir[] = GATO_LIBS_FRONT_OFFICE.'views'.DS.'smarty_plugins'.DS;
 ...
 */
	function _getLayoutFileName($name = null) {
		...
		foreach($paths->viewPaths as $path) {
			if (file_exists($path . 'layouts' . DS . $this->subDir . $type . $name . $this->ext)) {
				$layoutFileName = $path . 'layouts' . DS . $this->subDir . $type . $name . $this->ext;
				return $layoutFileName;
			}
		}
		...
		foreach($paths->viewPaths as $path) {
			if (file_exists($path . 'layouts' . DS  . $type . $name . '.ctp')) {
				$layoutFileName = $path . 'layouts' . DS . $type . $name . '.ctp';
				return $layoutFileName;
			}
		}

		if($layoutFileName = fileExistsInPath(LIBS . 'view' . DS . 'templates' . DS . 'layouts' . DS . $type . $name . '.ctp')) {
		} else {
			$layoutFileName = LAYOUTS . $type . $name.$this->ext;
		}
		return $layoutFileName;
	}
